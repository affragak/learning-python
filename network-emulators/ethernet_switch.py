...
def incoming_frame(src_mac, dst_mac, ingress_port, size):
    print(f"⚠️  Received frame on {ingress_port} from {src_mac} to {dst_mac}")
    ...
    if dst_mac in mac_table:
        ...
        print(f"  🟢  Address {dst_mac} found in MAC table on {egress_port}")
        forward_frame(dst_mac, egress_port, size)
    elif dst_mac == "FF:FF:FF:FF:FF:FF":
        print(f"  🟡 Destination is broadcast")
        flood_frame(dst_mac, ingress_port, size)
    else:
        print(f"  🔴 Address {dst_mac} NOT found in MAC table")
        flood_frame(dst_mac, ingress_port, size)
